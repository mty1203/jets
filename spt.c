...
void spt()
{
 ...
 for (int q=0;q<3;q++)
   ...
   for (double i=0.06;i<=0.82;i+=0.02)
   {
     values.push_back(i);
   }
   for (int j=0;j<values.size();j++)
   {
     ...
     {for (int c = 0;c<3;c++)
       ...
       cout<<"threshold = "<<values.at(j)<<" "<<q<<" "<<c<<" "<<result[q][c]<<endl;}}
   }
   TGraph *re[3][3];
...
for (int ieta=0;ieta<3;ieta++) 
  {
    for (int ipt=0;ipt<3;ipt++) 
    {
      ...
      stringstream mean;
     // re[ieta][ipt]->GetHistogram()->SetMaximum(100.);   // along          
      //re[ieta][ipt]->GetHistogram()->SetMinimum(0.);  
      re[ieta][ipt]->GetXaxis()->SetTitle("Threshold");
      re[ieta][ipt]->GetYaxis()->SetTitle("Number of Jets");
      re[ieta][ipt]->Draw();
    }
  }*/
...
void s(double threshold)
{
      ...
      for(int jhigh=0;jhigh<3;jhigh++)
       {
        ...
        fraction[ihigh][jhigh] = new TH2D(histname,"",100,0,high[jhigh],100,0,high[jhigh]);
   ...
   for(int ihigh=0;ihigh<3;ihigh++)
    {
      for(int jhigh=0;jhigh<3;jhigh++)
       {
         sprintf(histname, "h_x_%d_%d",ihigh,jhigh); 
        number[ihigh][jhigh] = new TH1D(histname,"",100,0.,100.);
  ...
  while(getline(myfile, line)) 
  {
    ...
    else
    {
     double w, x, y, z,q;
     int m;
     sscanf(line.c_str(),"%lf %lf %lf %lf %lf",&w,&x,&y,&z,&q);
     if(q<=threshold)
      {
       for (int idex=0;idex<3;idex++)
        {for(int jdex=0;jdex<3;jdex++)
           {
             if((w>=ptlow[jdex]&&w<pthigh[jdex])&&(x>=etalow[idex]&&x<etahigh[idex]))
                fraction[idex][jdex]->Fill(y,w);
                number[idex][jdex]->Fill(y);
           }
        }
      }
    }  
  } 
int sqentries[3][3]={15526,85,0,31355,7869,646,26490,8293,690};
TCanvas *c1 = new TCanvas("c1");
c1->Divide(3,3);
int k=0;
for (int ieta=0;ieta<3;ieta++) 
  {
    for (int ipt=0;ipt<3;ipt++) 
    {
      ...
      //re[ieta][ipt]->GetHistogram()->SetMinimum(0.);  
      fraction[ieta][ipt]->GetXaxis()->SetTitle("p_{t}^{jets}");
      fraction[ieta][ipt]->GetYaxis()->SetTitle("p_{t}^{struck quark}");
      fraction[ieta][ipt]->Draw("colz");
      TLatex latex;
      latex.SetTextAlign(13);  //align at top
      //mean<<"s_q entries = "<<fraction[ieta][ipt]->GetEntries();
      mean<<"s_q entries = "<<sqentries[ieta][ipt];
      TPaveText *pave = new TPaveText(.5,.2,.8,.45,"brNDC");
      pave->SetTextAlign(12);
      char temp[20];
      //sprintf(temp,"sq_entries= %5.0f",fraction[ieta][ipt]->GetEntries());
      sprintf(temp,"sq_entries= %d",sqentries[ieta][ipt]);
  ...
  for (int ieta=0;ieta<3;ieta++) 
  {
    for (int ipt=0;ipt<3;ipt++) 
    {
      double mean =  number[ieta][ipt]->GetEntries() ;
