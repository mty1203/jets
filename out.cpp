  ...
  //  if (verbosity>1) cout << "jet " << ijet << " (pt mass eta phi): "<< jets[ijet].pt() << " " << jets[ijet].m() << " "  << jets[ijet].rap() << " " << jets[ijet].phi() << endl;
   ...
   // match the constitutes to TTree indices
    ...
    {   
      ...
      //   }
       if (track_index>0)
        {   
         ...
         if(evt->GetTrack(track_index-1)->GetPt()>max_pt)
         {
           max_pt = evt->GetTrack(track_index-1)->GetPt();
           max_id = evt->GetTrack(track_index-1)->Id();

         }
            //TParticlePDG *
            // if(deltaR<0.3&&(quark->Id()>3||quark->Id()<-3)&&jets_pt>=5)
            // {myfile<<quark->Id()<<" "<<evt->GetTrack(track_index-1)->GetPt()<<" "<<evt->GetTrack(track_index-1)->GetRapidity()<<" "<<evt->GetTrack(track_index-1)->GetPhi()<<" "<<track_id<<" "<<deltaR<<" "<<jets_pt<<" "<<jets_eta<<endl;}
      ...
      myfile<<quark->Id()<<" "<<deltaR<<" "<<max_id<<" "<<max_pt<<" "<<jets_pt<<endl;
